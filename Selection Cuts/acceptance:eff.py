...
with hep.open(hepmc_file) as f:
    ...
    #histogram1 = TH1F("h1", "Histogram", 20000, 0, 200)
    #histogram2= TH1F("h2", "Histogram", 20000, 0, 200)

    # Fill the histogram with your single value
    n = good_event
    d = 12135
    histogram1.Fill(n)  # Fill the center of the single bin with your value
    hhistogram2.Fill(d+n)

    # Create a TEfficiency object using the histogram
    eff = TEfficiency(histogram1, histogram2)

    # Get the efficiency and uncertainties for the specific bin (in this case, bin 1)
    efficiency = eff.GetEfficiency(1)
    uncertainty_up = eff.GetEfficiencyErrorUp(1)  # Corrected this line by adding the bin number
    uncertainty_down = eff.GetEfficiencyErrorLow(1)  # Also corrected this line by adding the bin number

    print("Efficiency:", efficiency)
    print("Uncertainty (Up):", uncertainty_up)
    print("Uncertainty (Down):", uncertainty_down)
    
    effi=(0.35621+0.35519) /2 #eff at log -1.5 ns and log -0.5 ns
    final= n*effi
    print(final)
    print(final/20000)
